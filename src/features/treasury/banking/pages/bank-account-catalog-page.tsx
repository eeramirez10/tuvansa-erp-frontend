...
export function BankAccountCatalogPage() {
  ...
  const [panel, setPanel] = useState<BankPanelDefinition | null>(null)
  ...
  return <section className="mx-auto flex w-full min-w-0 max-w-[1800px] flex-1 flex-col gap-2">
    ...
    <div className="grid min-w-0 items-start gap-2 xl:grid-cols-[10rem_minmax(0,1fr)]"><aside><BankActionButtons onSelect={setPanel} /></aside><BankAccountDetails account={account} /></div>
    {searchOpen && <BankAccountSearchDialog onOpenChange={setSearchOpen} onSelect={(selected) => { setSearchOpen(false); setNotice(null); void navigate(paths.bankAccount(selected.id)) }} />}
    {panel && <BankPanelDialog account={account} key={`${account.id}-${panel.key}`} onOpenChange={(value) => { if (!value) setPanel(null) }} panel={panel} />}
  </section>
}
