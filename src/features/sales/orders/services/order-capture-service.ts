...
import type { CaptureCustomer, CaptureCustomerMatch, CaptureInput, CaptureOptions, CaptureProduct } from "@/features/sales/orders/capture-model"
...
export async function searchCaptureCustomers(query: string, signal?: AbortSignal) {
  return (await apiClient.get<{data: CaptureCustomerMatch[]}>("/sales/orders/capture/customers", { params: { query, limit: 50 }, signal })).data.data
}
